...
class OverallLoss(torch.nn.Module):
    ...
    def forward(self, output_dict: Dict[str, torch.Tensor], epoch) -> Dict[str, torch.Tensor]:
        l_det = self.prob_chamfer_loss(output_dict) #loss p
        l_spot,l_feat = self.spot_matching_loss(output_dict) #loss s c 用于coarse matching阶段
        ...
        l_desc, l_ov, l_corr = self.kpt_matching_loss(output_dict) #loss f k i
        l_trans = torch.norm(output_dict['transform'][:3, 3] - output_dict['gt_transform'][:3, 3]) #loss translation
        l_rot = torch.norm(output_dict['transform'][:3, :3].T @ output_dict['gt_transform'][:3, :3] - self.I3x3) #loss rotation 这两个用于fine matching阶段
