...
class KPConvHPA(nn.Module):
    def __init__(self, cfg):
        super(KPConvHPA, self).__init__()
        self.cfg = cfg
        init_dim = cfg.init_dim * 2
        init_sigma = cfg.init_sigma
        init_radius = cfg.init_radius

        # Encoder layer 1 (high resolution point cloud initial)
        self.encoder1_1 = ConvBlock(cfg.input_dim, cfg.init_dim, cfg.kernel_size, init_radius, init_sigma)
        self.encoder1_2 = ResidualBlock(cfg.init_dim, cfg.init_dim * 2, cfg.kernel_size, init_radius, init_sigma)
        ...
        # Generate key points and feature centers of gravity
        self.detector = KeypointDetector(32, cfg.init_dim * 4, cfg.init_dim)
        # Extract feature descriptors based on key points
        self.desc_extractor = DescExtractor(cfg.init_dim * 4, cfg.init_dim)
    
    def forward(self, points_list, neighbors_list, subsampling_list, upsampling_list):
        """
        `points_list`：Hierarchical point cloud pyramid
        `neighbors_list`：The geometric information of the neighborhood point index of each layer
        `subsampling_list`：High resolution -> Low resolution sampling information
        `upsampling_list`：Low resolution -> High resolution sampling information
        """
        ...
        # (2) Store features for HPA
        feats_list = []  
        
        # (3) Encoder layer by layer feature extraction
        for i in range(self.cfg.kpconv_layers - 1):
            feats = self.encoder[i][0](feats, points_list[i + 1], points_list[i], subsampling_list[i])  
            feats = self.encoder[i][1](feats, points_list[i + 1], points_list[i + 1], neighbors_list[i + 1])  
            feats = self.encoder[i][2](feats, points_list[i + 1], points_list[i + 1], neighbors_list[i + 1])  
        ...
        ########### **Decoder** ##########
        for i in range(1, self.cfg.kpconv_layers - 1):
            # Feature extraction for HPA
            feats_list[-i - 1] = self.decoder[i - 1](
                feats_list[-i - 1],  
                feats_list[-i],     
                upsampling_list[-i]  
            )

        ########### **Geomotery consistency detection for HPA** ##########
        xyz, sigma, grouped_feat, attentive_feat = self.detector(
            points_list[-1],  
            points_list[1],   
            feats_list[0],    
        )
