...
if __name__ == "__main__":
    ...
    if _args.mode == 'reg':
        # 1. Combine both point clouds (ref_cloud and src_cloud) into one PLY file
        ...
        # Combine colors for both clouds
        ref_colors = np.asarray(ref_cloud.colors)
        src_colors = np.asarray(src_cloud.colors)
        combined_colors = np.vstack((ref_colors, src_colors))
        combined_cloud.colors = o3d.utility.Vector3dVector(combined_colors)
        
        # Save the initial combined point cloud to a PLY file
        o3d.io.write_point_cloud("initial_combined_cloud_300.ply", combined_cloud)
        print("Saved initial_combined_cloud.ply containing both point clouds before transformation")

        # 2. Apply the transformation to the source cloud
        src_cloud.transform(trans)  # Apply the transformation to the source point cloud
        
        # 3. Combine the transformed source cloud with the reference cloud
        combined_cloud = o3d.geometry.PointCloud()
        ref_points = np.asarray(ref_cloud.points)
        src_points = np.asarray(src_cloud.points)
        combined_points = np.vstack((ref_points, src_points))
        ...
        # Combine colors after transformation
        ref_colors = np.asarray(ref_cloud.colors)
        src_colors = np.asarray(src_cloud.colors)
        combined_colors = np.vstack((ref_colors, src_colors))
        combined_cloud.colors = o3d.utility.Vector3dVector(combined_colors)
        
        # Save the combined point cloud after transformation to a new PLY file
        o3d.io.write_point_cloud("transformed_combined_cloud_300.ply", combined_cloud)
        print("Saved transformed_combined_cloud.ply containing both transformed source and reference clouds")
        ...
        o3d.io.write_triangle_mesh("ref_keypts.ply", ref_keypts)
        o3d.io.write_triangle_mesh("src_keypts.ply", src_keypts)
        print("Saved ref_keypts.ply and src_keypts.ply")
